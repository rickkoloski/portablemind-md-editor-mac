...
private final class RenderVisitor {
    ...
    func walk(_ markup: any Markup) {
        ...
        case is Table:
            // D17: tables are emitted via TK1TableBuilder in
            // buildAttributedString (replacement step). The walker
            // doesn't tag table source ranges with delimiter/font
            // attributes — those would be discarded by the
            // replacement anyway.
            break
        default:
    ...
    // D17: visitTable removed. Tables are emitted as TK1 NSTextTable
    // attributed strings via TK1TableBuilder during the renderer's
    // buildAttributedString step. The pre-D17 implementation tagged
    // table source ranges with TableRowAttachment metadata for the
    // TK2 layout-manager delegate; that delegate is gone.
