...
## Phase 1 — Spike (Q1 answer, ~1 day time-boxed)

### Goal

Prove that `NSTextLayoutFragment.textLineFragments` can return fragments with custom (x, width, height) typographic bounds and that NSTextView's caret honors those bounds when the selected range maps into the corresponding source offsets.

### Spike scope

- New throwaway file `spikes/d12_cell_caret/` mirroring `spikes/d01_textkit2/`'s pattern.
- Smallest possible reproducer: one `NSTextView`, one hardcoded table paragraph, one `TableRowFragment` with a hand-computed `textLineFragments` override that returns two line fragments side-by-side.
- Click on each "cell," observe caret position + height. Type a character, observe where it lands in the source.

### Spike success criteria

- Caret draws at (x, y) matching the clicked cell's bounds, at natural font line height.
- Typing inserts at the correct source offset (inside the cell's substring).
- No crashes, no layout thrash when clicking between cells.

### Spike findings outcomes

- **Green** → primary path in §3.4 is valid. Proceed to Phase 2 with line-fragment mapping.
- **Yellow** (works partially — e.g., caret position correct but height not honored) → narrow the design; may need to combine line-fragment mapping with custom caret drawing override. Capture findings, update plan before proceeding.
- **Red** (line-fragment bounds not honored by NSTextView in any workable way) → pivot to NSTextField overlay. Rewrite §3.4 of the spec. No modal fallback — that's off the table per CD.

### Spike timebox

1 day. If still Yellow/Red at end of day, stop and reassess with CD rather than extending.
...
### Step 2: Per-cell line-fragment override in `TableRowFragment`

**Files:** `Sources/Editor/Renderer/Tables/TableRowFragment.swift`

Override `textLineFragments` to return one `NSTextLineFragment` per cell:

```swift
override var textLineFragments: [NSTextLineFragment] {
    guard let idx = attachment.cellContentIndex else { return [] }
    let layout = attachment.layout
    let cellRanges = layout.cellRanges[idx]
    // ... build line fragments with:
    //   - attributedString = source substring of cell's range
    //   - typographicBounds = (x: columnLeadingX[col], y: cellInset.top,
    //                          width: contentWidths[col], height: naturalLineHeight)
    //   - glyphOrigin accounting for alignment + inset
}
```

- Natural line height = `layout.bodyFont.ascender + abs(layout.bodyFont.descender) + layout.bodyFont.leading` (or headerFont for header row).
- For wrapped cell content, emit multiple line fragments per cell stacked vertically.
...
1. **Spike Red outcome** — line-fragment bounds override doesn't work. Triggers a plan revision to NSTextField overlay path. Plan rework ~2 days; implementation may extend.
