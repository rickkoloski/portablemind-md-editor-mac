...
| 2026-05-07 | **Q9 — API endpoint surface.** Phase 1 spike confirmed model_api side has everything we need (`reference-code/model_api/app/controllers/api/v1/llm_files_controller.rb`, read 2026-05-07): `POST /api/v1/llm_files` for create (multipart with `llm_file[file]` + `llm_file[directory_path]` + `llm_file[title]`), `PATCH /api/v1/llm_files/:id` for rename (`llm_file[title]=newName`) and move (`llm_file[directory_path]=newPath`), and `GET /api/v1/llm_files/directories?tree=true` for the tree picker (already used by D18). PATCH returns 422 on uniqueness collision (no auto-rename — matches our Q5). **Important correction from initial draft:** the move parameter is `directory_path` (a path string like `/projects/foo`), NOT a numeric `llm_directory_id` — earlier draft said id; corrected here. The Swift client (`PortableMindAPIClient`) currently only has `updateFile` (D19); phase 1 adds `createFile`, `renameFile`, `moveFile`. No backend changes required — D23 is single-repo (md-editor only). | RAK |
