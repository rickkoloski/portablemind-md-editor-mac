...
### 2.4 External command surface is a declarative table
- **Rule:** Every command the editor can perform from outside — CLI invocation, URL scheme event, future MCP tool call, future AppleScript / Services / Shortcuts handler — is declared exactly once, in a single module (`Sources/CommandSurface/`). External callers look up a command by identifier and invoke it through one choke point. Never inline a handler for an external event elsewhere in the codebase.
- **Reason:** Parallel to §2.3. Keeping the surface centralized means adding an MCP adapter (when warranted) is a one-file addition rather than a codebase sweep. It also produces a greppable inventory of "what can be driven from outside" for compliance, security review, and docs.
- **Consequence of violation:** Two external entry points that silently diverge; commands that work via the CLI but not the URL scheme (or vice versa) because handler logic forked; an MCP upgrade that becomes a multi-week refactor instead of a thin adapter.
- **Relationship to other internal dispatchers:** `CommandDispatcher` (D4) handles *internal* invocations (keyboard, toolbar) against an active editor. `CommandSurface` (D6) handles *external* invocations against the app as a whole (open a file into a tab, focus a tab, list open tabs, etc.). The two are cousins, not duplicates — internal commands operate on the active editor's state; external commands operate on the app's window/tab/file state. Where they overlap semantically (e.g., "apply a mutation to the active editor"), the external surface routes through the internal dispatcher — one canonical invocation path per command.
