...
- [x] **Typing past cell width** — source updates correctly, but the spike's fixed `cellWidth: 320` doesn't grow → text overflows visually. **NOT a production concern**: production's `TableLayout.contentWidths[col]` is already computed per-column from the widest cell content (capped at `maxCellWidth` ~320pt, with CT-driven wrapping above the cap). The merge replaces the spike's hardcoded `cell1X`/`cell2X`/`cellWidth` with `layout.columnLeadingX[col]` + `layout.contentWidths[col]`. Behavior matches markdown semantics (no fixed cell widths in source; rendered widths derived from content).
- [x] **Typing a literal `|`** — typed pipe restructures the row. **Markdown-semantics correct behavior:** allow it; the row gains a new cell. Spike refactored to N-cell support: `cellOriginX(forIndex:)` / `cellRect(forIndex:)` / `cellIndex(forOffset:in:)` / `cellIndex(forPointX:in:)` helpers replace the hardcoded 2-cell anchors. Drawing, click routing, caret navigation, Tab, arrow keys, Backspace/Delete, selection highlights — all loop over `row.cells`. Verified: typing `|` in row 1 splits it into 3 cells while row 2 stays 2-cell (mixed cell counts within the same table). Right-arrow chain across 3-cell row: 6→8→14→cross-row to 27. **No production-policy decision needed.** Markdown is truth; render whatever the source dictates.
- [x] **Empty cell** — `parseCellRanges` rewritten to record zero-length ranges instead of skipping empty cells. **Caret-snap fix:** `LoggingTextView.snapCaretToCellContent()` runs after every `mouseDown`. If the resulting selection is OUTSIDE any cell's content range (length 0 + offset not in `[cell.start, cell.start + cell.length]` for any cell), the caret snaps to the nearest cell-content-edge by source distance. Verified: clicking empty cell 1 → caret snaps from offset 4 to offset 3, typing "X" produces source `|  X|` and cell 1 now has content "X". **Production:** the same snap pattern works at the NSTextView subclass layer; can also be implemented as a `NSTextSelectionNavigation.destinationSelectionFor...` override for a more principled approach.
